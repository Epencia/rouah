...
export default function ListeDiplome({navigation,item}) {
...
const getPartenaire = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.cloud/api/liste-diplome.php?matricule=${carte[0].numero_carte}`, {
    headers: {
...
/>
  ...
  <View style={{marginTop: 25, marginRight:15,marginLeft:15,
    ...
    <FlatList
      ...
      keyExtractor={(item) => item.code_certificat}
      renderItem={({item}) => (
        <View style={styles.experienceItem}>
         
        <TouchableOpacity  onPress={() => navigation.navigate("Diplomes", { item })}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                 <MaterialCommunityIcons style={styles.image} color="#0A84FF" name="school" size={50} />

        <View style={styles.textContainer}>
          <Text style={styles.text}>{item.titre_formation? item.titre_formation : "Aucun résultat"}</Text>
          <View style={styles.dataContainer}>
            <Text style={styles.dataText}>{item.diplome ? item.diplome : "Aucun résultat"}</Text>
          </View>
        </View>
        </View>
