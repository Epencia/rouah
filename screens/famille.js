...
const MemberInfoCard = ({ member, getStatusColor, getBatteryColor }) => (
...
const MemberCard = ({ member, getStatusColor, getBatteryColor }) => (
...
const ZoneCard = ({ zone, familyMembers }) => (
...
export default function MaFamille() {
  ...
  const MapTab = () => (
    <ScrollView 
      contentContainerStyle={styles.mapTabContainer}
  ...
  const ZonesTab = () => (
    <FlatList
      data={safeZones}
      renderItem={({ item }) => <ZoneCard zone={item} familyMembers={familyMembers} />}
      keyExtractor={item => item.id.toString()}
